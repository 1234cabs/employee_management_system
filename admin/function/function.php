...
if(isset($_POST['create'])) {
    ...
    empty($Uname) || empty($pos) || empty($role) || empty($password)){
    ...
    $stmt->bind_param("sssssss", $Fname,$email,$contact,$Uname,
    $pos,$role,$pass);
    $stmt->execute();
...
// }
    // $updated_at = date("Y-m-d H:i:s");  // 🔥 ADD THIS
    ...
    $stmt = $conn->prepare("UPDATE users SET fullname=?, email=?, contact=?,
    username=?, position=? WHERE id=?");
    $stmt->bind_param("sssssi", $Fname,$email,$contact,$Uname,$pos,$id);
    $stmt->execute();
    if($stmt->affected_rows == 0){
        $_SESSION['error'] = "No Changes Made!";
    }else{
        $_SESSION['success'] = "This Record Successfully Updated";
    }
    header("Location: ../admin.php");
    exit;
